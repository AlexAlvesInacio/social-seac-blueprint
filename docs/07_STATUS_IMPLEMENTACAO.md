...
| Assistidos | Migrado ao Supabase | Criação via `criar_assistido_em_familia` (com reuso de pessoa existente, `p_pessoa_id`) e leitura no agregado da família. Aprovação manual Extra → Definitivo via `aprovar_assistido_definitivo` (no atendimento, estado "Extra completou"). |
