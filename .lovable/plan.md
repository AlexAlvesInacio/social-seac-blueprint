...
- **`tentativas_entrega`** — id, assistido_id, familia_id, usuario_id (quem tentou), data_hora, **tipo_cesta** (`extra`|`padrao`), motivo_bloqueio (`antes_25_dias`|`sem_estoque`|`assistido_inativo`|`outro`), proxima_data_permitida, observacao, status (`bloqueada`|`liberada_excepcionalmente`|`cancelada`), **liberado_por_usuario_id** (FK, nullable), **liberado_em** (nullable), **observacao_liberacao** (nullable).
